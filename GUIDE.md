...
An authenticator is responsible for authenticating a users given credentials (often their email and password) with an API.

For this app you'll write an authenticator that will send the email and password entered by the user to the `Api::V1::SessionsController` you wrote earlier.

When authentication is successful, the sessions controller responds with the user's API token. The authenticator will store the API token in the client browser. The app will submit this API token in the Authorization header of all subsequent requests sent to the API.

The authenticator you're writing is going to follow the structure expected and generated by the Simple Auth addon. Run the authenticator generator that Simple Auth provides:

```bash
bin/ember generate authenticator api-v1
```
...
Open the authenticator `client/app/authenticators/api-v1.js` and update its contents to be:
...
Review the authenticator code and notice it makes an AJAX POST request to the `/api/v1/sessions` endpoint you added to the Rails app earlier. This will call the `create` action of `Api::V1::SessionsController`.

The `login-form` component is going to use the `api-v1` authenticator instead of the `veryInsecureAuthenticationSucceeds` placeholder function you wrote earlier.

Update `client/app/pods/components/login-form/component.js`:

```diff
--- a/client/app/pods/components/login-form/component.js
+++ b/client/app/pods/components/login-form/component.js
@@ -21,28 +21,21 @@ export default Ember.Component.extend({
       var notifier = this.get('notifier');
...
In the above changes, `this.get('session')` is a session object injected into the component by Simple Auth.

You then call the `authenticate('authenticator:api-v1', credentials)` function on the session object. This instructs Simple Auth to try authenticating with the `api-v1` authenticator you wrote, by sending the `credentials` entered by the user.

---

**INSTRUCTION FOR DAN START**

Delete the following line from the end of `spec/features/user_registration_spec.rb`:
...
## Simple Auth's `routeAfterAuthentication`

After logging-in successfully a user is transitioned to the `list.index` route by the `login-form` component.

You're going to remove the code responsible for this from the component. To replace it you're going to configure Simple Auth's `routeAfterAuthentication` setting to equal `list` (`list` and `list.index` are treated as the same routes by Ember). Simple Auth defaults to redirecting users who have just logged in to the route named by `routeAfterAuthentication`.

Remove the route transition code from `client/app/pods/components/login-form/component.js`:

```diff
--- a/client/app/pods/components/login-form/component.js
+++ b/client/app/pods/components/login-form/component.js
@@ -25,7 +25,4 @@ export default Ember.Component.extend({
       var onAuthentication = () => {
         notifier.setMessage("You are signed in.");
-
-        // Show the list index when login is successful:
...
Add the `routeAfterAuthentication` setting to the existing Simple Auth environment configuration in `client/config/environment.js`:
...
Run the user registration feature spec:
...
TODO NEXT:
0. Show "Saving..." message and disable button when saving a new list. list.isDirty vs list.hasDirtyAttributes
1. Feature Spec for new list
